...
| Reborn | `undead-reborn` | `REB_1`…`10` | Guardian | Ghastly Ward | Pale Bulwark | Wraithwall Eternal | 1111–1120 | Timed MP-weighted shield pulse. Beginning: pulse every 60s (wards 1001–1003). Middle: pulse accelerates 55s→30s (wards 1004–1009). Capstone: pulse every 15s, 3-stack `shieldProtect` ward. | ✅ two-layer — [Reborn reference](#reference-reborn--ghastly-ward-undead-reborn) |
...
| Skeleton | `undead-skeleton` | `BON_1`…`10` | Berserker | Undying Rage | Graveborn Fury | Deathless Fury | 1131–1140 | Below low HP threshold, grants ATK+MAT via payload states (1011–1020). Beginning: +1/2/3% at ≤20% HP. Middle: +3% ATK/MAT, threshold stays 20%, payload scales. Capstone: +5% ATK/MAT/−50% PDR/MDR, all debuff immunity. | ✅ `<passiveStateCount:[ID, lessIsMoreHp, MULT]>` + payload ATK/MAT buff tags |
| Armor | `undead-armor` | `ARM_1`…`10` | Vanguard | Hollow Armor | Brittle Bastion | Paper Fortress | 1141–1150 | High DEF, lower effective MHP. Beginning: +6/9/12% DEF, −2/4/6% MHP (traits). Middle: +28→30% DEF, −15% MHP + `<grdBuffRate>` scaling 10→30%. Capstone: +100% DEF, −50% MHP, `<grdBuffRate:50>` + `<critReduction:30>` + `<cdrBuffRate:50>`. | ✅ DEF↑ / MHP↓ traits + `<grdBuffRate>` |
...
| Crab | `aquatic-crab` | Guardian | Iron Shell | Iron Rebuke | Shellbreaker Retort | 1221–1230 | Physical retaliate on every hit (100% proc). Tiers escalate payload damage (1011–1020). | ✅ `<retaliate:[ID, 100, physical]>` · payloads **1011–1020** |
...
| Subgroup | `subgroupKey` | Archetype | **1–3** | **4–9** | **10** | IDs | What it does | Tag recipe |
|---|---|---|---|---|---|---:|---|---|
| Puddle | `slime-puddle` | Generalist | Adaptive Slime | Reactive Gel | Elemental Osmosis | 1251–1260 | On HP damage, apply Elemental Gel debuff (payloads 1051–1060). Beginning: 3s cooldown. Middle: 2s cooldown. Capstone: 1s cooldown. | ✅ `<autoApplyState:[105X, hpDmg, CD]>` |
...
| Aerial | `slime-aerial` | Cleric | Regeneration Aura | Miasma of Life | Spore Bloom | 1281–1290 | Periodic AoE heal pulse on nearby allies (payloads 1021–1030). Beginning: every 8s. Middle: every 7s→3s. Capstone: every 2s + personal 5% MHP regen per tick. | ✅ `<autoExecuteSkill:[102X, time, FRAMES]>` + `<hpPercent:5>` |
...
| Subgroup | `subgroupKey` | Archetype | **1–3** | **4–9** | **10** | IDs | What it does | Tag recipe |
|---|---|---|---|---|---|---:|---|---|
| Trap | `plant-trap` | Wizard | Entangling Curse | Thorned Curse | Stranglethorn Curse | 1301–1310 | Your debuffs last X% longer. Beginning: +10/20/30%. Middle: +50→100%. Capstone: +200% duration + +100% damage to poisoned targets. | ✅ `<stateDurationPerc:N>` / capstone + `<bonusDamageIfState:[5, 100]>` |
| Fungus | `plant-fungus` | Berserker | Primal Instinct | Primal Surge | Primal Apex | 1311–1320 | +X% ATK while all skills are off cooldown. Beginning: +10/20/30% ATK. Middle: +50% ATK + escalating flat ATK bonus (20→100%). Capstone: +255% ATK + 255% flat ATK bonus + 2 bonus basic hits. | ✅ `<passiveSourceRule:[allOffCooldown]>` + ATK x-param + ATK b-param traits |
...
| Bat | `beast-bat` | Skirmisher | Cauldron Instinct | Chittering Frenzy | Wingbeat Chorus | 1361–1370 | +X% HIT per nearby ally. | ✅ `<passiveSourceRule:[alliesNearby, N]>` |
| Beaker | `beast-beaker` | Artillery | Tailwind | Rising Gust | Gale Force | 1371–1380 | +X% MSB after using a skill. | ✅ `<passiveSourceRule:[attackedWithin, 180]>` |
| Rat | `beast-rat` | Generalist | Resourceful Rodent | Nest Egg | Compound Interest | 1381–1390 | More SDP and gold. | ✅ `sdr` + `gdr` |
| Quadruped | `beast-quadruped` | Guardian | Alpha Presence | Alpha Howl | Pack Devilty | 1391–1400 | Allies +X% DEF. | ✅ `<passiveSourceRule:[alliesNearby, N]>` |
...
| Parasite | `insect-parasite` | War Priest | Siphon Aura | Siphon Weave | Cradle of Leech | 1441–1450 | 1–3: HP splash 11/22/33% range 2. 4–9: HP splash 50→100% + MP splash 5→30% range 3. 10: HP 100% + MP 50% + TP 25% range 4. All MAX_DEPTH 1. | ✅ `<onSelfHpHealHp:[PCT, R, 1]>` · `<onSelfMpHealMp:[PCT, R, 1]>` · `<onSelfTpHealTp:[PCT, R, 1]>` |
...
| Subgroup | `subgroupKey` | Archetype | **1–3** | **4–9** | **10** | IDs | What it does | Tag recipe |
|---|---|---|---|---|---|---:|---|---|
| Titan | `construct-titan` | Berserker | Unstoppable | Relentless March | Juggernaut | 1501–1510 | KB resist; +ATK per self-debuff. | ✅ KB + `passiveStateCount` |
...
| Orb | `construct-orb` | Medic | Overcharge | Capacitor Surge | Meltdown | 1541–1550 | Shield break → AoE blast. | ✅ Shield-break explosion |
...
| Reborn | Ghastly Ward | Pale Bulwark | Wraithwall Eternal |
| Wisp | Blistering Aura | Searing Mantle | Scorched Halo |
| Skeleton | Undying Rage | Graveborn Fury | Deathless Fury |
| Armor | Hollow Armor | Brittle Bastion | Paper Fortress |
...
| Crab | Iron Shell | Iron Rebuke | Shellbreaker Retort |
| Fish | Slippery | Swift Current | Slipstream |
| Cephalopod | Ink Shroud | Murky Pall | Abyssal Veil |
| Puddle | Adaptive Slime | Reactive Gel | Elemental Osmosis |
| Roper | Eldritch Fury | Eldritch Tempest | Eldritch Maelstrom |
| Jelly | Mana Transfusion | Mana Weave | Arcane Transfusion |
| Aerial | Regeneration Aura | Miasma of Life | Spore Bloom |
| Cube | Living Obstacle | Living Bulwark | Immovable Bulk |
| Trap | Entangling Curse | Thorned Curse | Stranglethorn Curse |
| Fungus | Primal Instinct | Primal Surge | Primal Apex |
...
| Bat | Cauldron Instinct | Chittering Frenzy | Wingbeat Chorus |
| Beaker | Tailwind | Rising Gust | Gale Force |
| Rat | Resourceful Rodent | Nest Egg | Compound Interest |
| Quadruped | Alpha Presence | Alpha Howl | Pack Devilty |
...
| Titan | Unstoppable | Relentless March | Juggernaut |
...
| Orb | Overcharge | Capacitor Surge | Meltdown |
| Elemental | Elemental Saturation | Elemental Flux | Elemental Singularity |
| Emotion | Empathic Bond | Empathic Echo | Empathic Nexus |
| Devil | Devil's Bargain | Devil's Wager | Devil's Due |
...
## Reference: Reborn / Ghastly Ward (`undead-reborn`)
...
|---|---|---|---|
| **Mastery passive** | **1111–1120** (skill = state) | Passives menu act names (Ghastly Ward / Pale Bulwark / Wraithwall Eternal) | `<autoApplyState:[WARD_ID, time, FRAMES]>` on **this** row |
...
**Mastery tag shape:** single `<onCritApply:[PAYLOAD_ID, CHANCE]>`.
