...
var J_AbsPluginMetadata = class extends PluginMetadata {
	...
	*/
	initializeAiTuningMetadata() {
		this.AiComboHumanizeWindowMinPercent = .1;
		this.AiComboHumanizeWindowMaxPercent = .3;
		this.AiDefensiveDodgeChancePercent = 75;
		this.AiDefensiveDodgeCooldownFrames = 45;
		this.AiDefensiveThreatRadiusTiles = 3;
		this.AiAllyDefensiveGuardHpThresholdPercent = .55;
	...
	*/
	initializeHitboxOverlayStyleMetadata() {
		this.HitboxStyles = {
			...
			byShape: {
				circle: { fillColor: 16744272 },
				rhombus: { fillColor: 16766336 },
				square: { fillColor: 16754253 },
				line: { lineWidth: 3 },
				wall: { lineColor: 13395456 },
				cross: { fillAlpha: .25 },
				arc: { fillColor: 16758861 }
			},
			byKind: {
				player: {
					fillColor: 5088255,
				...
				follower: { fillColor: 10181046 },
				battler: { fillColor: 3066993 }
			},
			byState: { colliding: {
				fillColor: 16726832,
				fillAlpha: .35,
...
//#endregion
//#region src/plugins/abs/core/_metadata/initialization.js
/**
...
J.ABS.Helpers.PluginManager.TranslateOptionToSlot = (slot) => {
	switch (slot) {
		...
		case "L1B": return JABS_Button.CombatSkill2;
		case "L1X": return JABS_Button.CombatSkill3;
		case "L1Y": return JABS_Button.CombatSkill4;
	}
};
/**
...
*/
var JABS_HitboxPulseOptions = class JABS_HitboxPulseOptions {
	/**
	...
	*/
	static defaults() {
		...
		o.scaleStart = 1;
		o.scaleEnd = 1.08;
		o.lineColor = 16777215;
		o.lineAlpha = .85;
		o.lineWidth = 2;
		o.fillColor = 16777215;
		o.fillAlpha = .18;
	...
	*/
	static from(data, base) {
		...
		const seed = base ? base.clone() : JABS_HitboxPulseOptions.defaults();
		return seed.apply(data || {});
	}
};

//#endregion
//#region src/plugins/abs/core/sprites/Sprite_HitboxPulse.js
/**
...
//#endregion
//#region src/plugins/abs/core/managers/JABS_HitboxPulseManager.js
/**
...
*/
var JABS_HitboxPulseManager = class JABS_HitboxPulseManager {
	/**
	...
	*/
	static syncSustainedActionPulse(jabsAction) {
		...
		if (!pulse) {
			pulse = pool.length > 0 ? pool.pop() : new Sprite_HitboxPulse();
	...
	*/
	static releaseSustainedPulse(uuid) {
		...
		JABS_HitboxPulseManager.getPool().push(pulse);
	}
	static spawn(data) {
		const layer = JABS_HitboxPulseManager.getLayer();
		if (!layer) return;
		...
		const pulse = pool.length > 0 ? pool.pop() : new Sprite_HitboxPulse();
		pulse.reset();
	...
	*/
	static clear() {
		Object.keys(JABS_HitboxPulseManager._sustainedByUuid).forEach((uuid) => {
...
*/
var JABS_LocationBuilder = class {
	#x = null;
	#y = null;
	#d = null;
	build() {
	...
	*/
	faceReverse(targetLocation) {
		switch (targetLocation.d) {
			...
			case J.ABS.Directions.UP: return this.facingDown();
...
*/
var JABS_Location = class JABS_Location {
	/**
...
var JABS_AI = class {
	...
	* @returns {number}
	*/
	countLowHpAllies(healer, threshold = .6) {
		const nearbyAllies = healer.getAllNearbyAllies();
...
*/
var JABS_BattlerCoreDataBuilder = class {
	/**
	...
	*/
	isDummy(isFriendly = false) {
		this.#teamId = isFriendly ? JABS_Battler.allyTeamId() : JABS_Battler.enemyTeamId();
		return this;
	}
	/**
...
*/
var JABS_AiManager = class JABS_AiManager {
	/**
	...
	*/
	static getAllBattlers() {
		return Array.from(this.battlers.values());
	}
	/**
	...
	*/
	static getBattlerByEventId(eventId) {
		return this.getAllBattlers().find((battler) => battler.getCharacter().eventId() === eventId);
	}
	/**
	...
	*/
	static getBattlersWithinRange(user, maxRange) {
		return this.getAllBattlers().filter((battler) => user.distanceToDesignatedTarget(battler) <= maxRange);
	}
	/**
	...
	*/
	static anyLivingEnemiesAggroedToParty() {
		const enemies = JABS_AiManager.getEnemyBattlers().filter((enemy) => enemy.isDead() === false);
	...
	*/
	static convertEventsToBattlers(events) {
		return events.map((event) => this.convertEventToBattler(event)).filter((event) => !!event);
	}
	/**
	...
	*/
	static convertFollowersToBattlers(followers) {
		return followers.map(this.convertFollowerToBattler, this).filter((follower) => !!follower);
	}
	/**
	...
	*/
	static manageAi() {
		const battlers = this.getAllBattlersWithinRangeSortedByDistance($jabsEngine.getPlayer1(), J.ABS.Metadata.MaxAiUpdateRange);
	...
	*/
	static getGuardianWardAttacker(guardian) {
		...
		const nearbyWards = this.getAlliedBattlersWithinRange(guardian, scanRange).filter((ally) => ally.getBattlerRole().ward);
	...
	*/
	static getGuardianEffectivePursuitRadius(guardian) {
		...
		const maxWardPursuit = allAllies.filter((ally) => ally.getBattlerRole().ward).reduce((max, ward) => Math.max(max, ward.getPursuitRadius()), 0);
	...
	*/
	static setupActionForNextPhase(battler, skillId, cooldownKey) {
		...
		const optionsBuilder = JABS_ActionOptions.Builder().setCooldownKey(cooldownKey);
		if (skill.jabsDirect && !skill.jabsDirectLock) {
			const [x, y] = battler.resolveDirectActionTargetCoordinatesForSkill(skill);
			if (x !== null && y !== null) {
				const frozen = JABS_Location.Builder().setX(x).setY(y).build();
	...
	*/
	static needsToMoveCloser(battler) {
		...
		const distanceToTarget = battler.getAllyTarget() ? battler.distanceToAllyTarget() : battler.distanceToCurrentTarget();
	...
	*/
	static needsAxisAlignment(battler) {
		...
		const isNarrowShape = shape === J.ABS.Shapes.Line || shape === J.ABS.Shapes.Wall || shape === J.ABS.Shapes.Arc;
		...
		const misalignment = absDx >= absDy ? Math.abs(ty - by) : Math.abs(tx - bx);
		let tolerance;
...
//#endregion
//#region src/plugins/abs/core/managers/JABS_TeamRules.js
/**
...
var JABS_TeamRules = class {
	...
	*/
	static isOpposed(teamA, teamB) {
		...
		const teamDefinition = this.teams().find((team) => team.id === teamA);
		if (!teamDefinition) {
			return true;
		}
...
*/
var JABS_InputAdapter = class JABS_InputAdapter {
	/**
...
var JABS_Engine = class JABS_Engine {
	...
	*/
	static initializeEnemyMap() {
		...
		fetch(`data/${mapFilename}`).then((data) => data.json()).then((dataMap) => JABS_Engine.setEnemyCloneList(dataMap.events));
	}
	/**
	...
	*/
	static resolveMeleeVerticalLiftPxForFacing(facing) {
		...
		const minLift = Math.max(8, th * .18);
	...
	*/
	static #defensivePressure(caster, target, ignoreParryPercent) {
		...
		const tenPercent = (value) => parseFloat((value * .1).toFixed(3));
	...
	*/
	shouldHandleDefeatedTarget(target) {
		...
		if (target.isEnemy() && target.getCharacter().isErased()) {
			return false;
		}
		return true;
	}
	...
	*/
	rotate45degrees(direction, clockwise) {
		...
		switch (direction) {
			case J.ABS.Directions.UP:
				newDirection = clockwise ? J.ABS.Directions.UPPERRIGHT : J.ABS.Directions.UPPERLEFT;
				break;
			case J.ABS.Directions.RIGHT:
				newDirection = clockwise ? J.ABS.Directions.LOWERRIGHT : J.ABS.Directions.UPPERRIGHT;
				break;
			case J.ABS.Directions.LEFT:
				newDirection = clockwise ? J.ABS.Directions.UPPERLEFT : J.ABS.Directions.LOWERLEFT;
				break;
			case J.ABS.Directions.DOWN:
				newDirection = clockwise ? J.ABS.Directions.LOWERLEFT : J.ABS.Directions.LOWERRIGHT;
				break;
			case J.ABS.Directions.LOWERLEFT:
				newDirection = clockwise ? J.ABS.Directions.LEFT : J.ABS.Directions.DOWN;
				break;
			case J.ABS.Directions.LOWERRIGHT:
				newDirection = clockwise ? J.ABS.Directions.DOWN : J.ABS.Directions.RIGHT;
				break;
			case J.ABS.Directions.UPPERLEFT:
				newDirection = clockwise ? J.ABS.Directions.UP : J.ABS.Directions.LEFT;
				break;
			case J.ABS.Directions.UPPERRIGHT:
				newDirection = clockwise ? J.ABS.Directions.RIGHT : J.ABS.Directions.UP;
				break;
		}
		return newDirection;
	}
	...
	*/
	isParryPossible(caster, target) {
		...
		if (caster.getBattler().ignoreAllParry()) {
			return false;
		}
		return true;
	}
	...
	*/
	createAttackLog(action, target) {
		...
		if (result.hpDamage) {
			...
			roundedDamage = roundedDamage >= 0 ? roundedDamage : roundedDamage.toString().replace("-", String.empty);
	...
	*/
	getCollisionTargets(jabsAction) {
		...
		candidates.filter(canActionConnectWithBattler, this).forEach(battlerCollisionProccessor, this);
		return targetsHit;
	}
	/**
	...
	*/
	isTargetWithinRange(facing, targetCharacter, actionEvent, range, shape) {
		switch (shape) {
			...
			case J.ABS.Shapes.Square: return this.collisionSquare(targetCharacter, actionEvent, range);
			case J.ABS.Shapes.Cross: return this.collisionCross(targetCharacter, actionEvent, range);
			case J.ABS.Shapes.Line: return this.collisionLine(targetCharacter, actionEvent, range, facing);
			case J.ABS.Shapes.Arc: {
	...
	*/
	collisionSector(target, action, range, facing, degrees) {
		...
		const th = $gameMap.tileHeight();
		const rPx = range * tw;
		const { x: cx, y: cy } = JABS_Engine.getActionOriginPixels(action);
		...
		const halfAngleRad = degrees * .5 * (Math.PI / 180);
		...
		const samplePoints = [
			...
			{
				px: targetRect.x + targetRect.w,
				py: targetRect.y
			},
			{
				px: targetRect.x,
				py: targetRect.y + targetRect.h
			},
			{
				px: targetRect.x + targetRect.w,
				py: targetRect.y + targetRect.h
	...
	*/
	collisionCircle(target, action, range) {
		const tw = $gameMap.tileWidth();
		const rPx = range * tw;
		const { x: originCx, y: originCy } = JABS_Engine.getActionOriginPixels(action);
		const targetRect = JABS_Engine.getBattlerAabbModel(target);
	...
	*/
	gainExperienceReward(experience, casterCharacter) {
		...
		$gameParty.battleMembers().forEach((member) => member.gainExp(experience));
	}
	/**
	...
	*/
	configureLevelUpLog(targetName, newLevel) {
		return new ActionLogBuilder().setupLevelUp(targetName, newLevel).build();
	}
	/**
	...
	*/
	configureSkillLearnLog(targetName, learnedSkillId) {
		return new ActionLogBuilder().setupSkillLearn(targetName, learnedSkillId).build();
	}
};
...
var JABS_ActionBuilder = class {
	...
	*/
	build() {
		const mapAction = new JABS_Action(this.#gameAction, this.#caster, this.#isRetaliation, this.#initialDirection, this.#cooldownKey, this.#isTerrainDamage);
...
*/
var JABS_Action = class JABS_Action {
	/**
	...
	*/
	...
	isHealing() {
		...
		return [3, 4].includes(damageType);
	}
	/**
	...
	*/
	static collectSyntheticVisualNoteFromActionEventPage(eventData, pageData) {
		...
		Game_Event.getValidCommentCommandsFromPage(pageData).forEach((command) => {
	...
	*/
	syncDirectActionSpriteToCaster() {
		...
		const frozenLocation = options ? options.getTargetLocation() : null;
		const frozenX = frozenLocation ? frozenLocation.getX() : null;
		const frozenY = frozenLocation ? frozenLocation.getY() : null;
		if (frozenX !== null && frozenY !== null) {
			return;
		}
		const casterChar = this.getCaster().getCharacter();
	...
	*/
	composeHitboxPulsePlainOptions() {
		...
		if (actionEvent) {
			...
			const casterCharacter = this.getCaster().getCharacter();
			const o = JABS_Engine.getMeleeVisualOriginPixelsFromCharacter(casterCharacter);
			originX = o.x;
			originY = o.y;
			facing = casterCharacter.direction();
		}
		const degrees = actionEvent ? $jabsEngine.getActionDegrees(actionEvent) || 180 : 180;
		const thickness = actionEvent ? $jabsEngine.getActionThicknessTiles(actionEvent) || 1 : 1;
		const useFade = meta.useFadeAnimation === true;
		const duration = useFade ? meta.duration : 999999;
		const endAlpha = useFade ? meta.endAlpha : meta.startAlpha;
		const scaleEnd = useFade ? meta.scaleEnd : meta.scaleStart;
		return {
			x: originX,
			y: originY,
			shape: this.getShape(),
...
PluginManager.registerCommand(J.ABS.Metadata.name, "Set JABS Skill", (args) => {
	...
	if (skillSlotKey === JABS_Button.Offhand) {
		...
		if (isLocked) {
			actor.getSkillSlotManager().getSkillSlotByKey(JABS_Button.Offhand).lock();
		}
		return;
	}
...
//#endregion
//#region src/plugins/abs/core/database/RPG_BaseBattler.js
/**
* Bonus hits per connection from this battler database note, applied to all JABS actions.
* @type {number}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsBonusHitsScopeGlobal", { get: function() {
...
* @type {number}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsBonusHitsScopeBasic", { get: function() {
...
* @type {number}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsBonusHitsScopeSkill", { get: function() {
...
* @type {number}
*/
Object.defineProperty(RPG_Class.prototype, "jabsBonusHitsScopeGlobal", { get: function() {
	return RPGManager.getNumberFromNoteByRegex(this, J.ABS.RegExp.BonusHitsScopeGlobal);
} });
/**
...
* @type {number}
*/
Object.defineProperty(RPG_Class.prototype, "jabsBonusHitsScopeBasic", { get: function() {
	return RPGManager.getNumberFromNoteByRegex(this, J.ABS.RegExp.BonusHitsScopeBasic);
} });
/**
...
* @type {number}
*/
Object.defineProperty(RPG_Class.prototype, "jabsBonusHitsScopeSkill", { get: function() {
	return RPGManager.getNumberFromNoteByRegex(this, J.ABS.RegExp.BonusHitsScopeSkill);
} });
/**
...
//#endregion
//#region src/plugins/abs/core/database/RPG_Enemy.js
/**
* The JABS team id for this battler.
* This number is the id of the team that this battler will belong to.
* @type {number}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsTeamId", { get: function() {
...
* @returns {number|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsPrepareTime", { get: function() {
...
* @returns {number|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsSightRange", { get: function() {
...
* @returns {number|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsPursuitRange", { get: function() {
...
* @returns {number|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsGuardRange", { get: function() {
...
* @returns {number|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsAlertDuration", { get: function() {
...
* @returns {number|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsAlertedSightBoost", { get: function() {
...
* @returns {number|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsAlertedPursuitBoost", { get: function() {
...
* @type {JABS_EnemyAI}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsBattlerAi", { get: function() {
...
* @type {JABS_BattlerRole}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsBattlerRole", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsAiTraitCareful", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsAiTraitExecutor", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsAiTraitReckless", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsAiTraitHealer", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsAiTraitFollower", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsAiTraitLeader", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsAiTraitCleanser", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsAiTraitBuffer", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsAiTraitTactical", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsAiTraitBerserker", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsAiRoleLeader", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsAiRoleFollower", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsAiRoleGuardian", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsAiRoleWard", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsAiRoleSolo", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsAiRoleSentinel", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsConfigCanIdle", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsConfigNoIdle", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsConfigNoHpBar", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsConfigShowName", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsConfigNoName", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsConfigInvincible", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsConfigNotInvincible", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsConfigInanimate", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_BaseBattler.prototype, "jabsConfigNotInanimate", { get: function() {
	return RPGManager.checkForBooleanFromNoteByRegex(this, J.ABS.RegExp.ConfigNotInanimate, true);
} });

//#endregion
//#region src/plugins/abs/core/database/RPG_EquipItem.js
/**
* The skill id associated with this equipment.
* This is typically found on weapons and offhand armors.
* @type {number|null}
*/
Object.defineProperty(RPG_EquipItem.prototype, "jabsSkillId", { get: function() {
...
* @type {false}
*/
Object.defineProperty(RPG_EquipItem.prototype, "jabsUseOnPickup", { get: function() {
	return false;
} });
/**
...
* @type {number|null}
*/
Object.defineProperty(RPG_EquipItem.prototype, "jabsExpiration", { get: function() {
...
//#endregion
//#region src/plugins/abs/core/database/RPG_Item.js
/**
* The skill id associated with this item or tool.
* @type {number|null}
*/
Object.defineProperty(RPG_Item.prototype, "jabsSkillId", { get: function() {
	return RPGManager.getNumberFromNoteByRegex(this, J.ABS.RegExp.SkillId, true);
} });
/**
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_Item.prototype, "jabsUseOnPickup", { get: function() {
...
* @type {number}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsRadius", { get: function() {
...
* @type {number}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsProximity", { get: function() {
...
* @type {number}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsActionId", { get: function() {
...
* @type {number}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsDuration", { get: function() {
...
* @type {number}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsLinger", { get: function() {
...
* @type {string}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsShape", { get: function() {
...
* @type {number|null}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsKnockback", { get: function() {
...
* @type {number}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsCastAnimation", { get: function() {
...
* @type {boolean}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsDirect", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsDirectLock", { get: function() {
...
* @type {number|null}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsDirectStateTarget", { get: function() {
...
* @type {number}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsBonusAggro", { get: function() {
...
* @type {number}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsAggroMultiplier", { get: function() {
...
* @type {number[]}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsCounterGuard", { get: function() {
...
* @type {number[]}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsCounterParry", { get: function() {
...
* @type {number}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsProjectile", { get: function() {
...
* @type {string}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsProjectileFormation", { get: function() {
...
* @type {number}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsDodgeSteps", { get: function() {
...
* @type {number}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsDodgeSpeed", { get: function() {
...
* @type {string}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsMoveType", { get: function() {
...
* @type {boolean}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsInvincibleDodge", { get: function() {
...
* @type {boolean}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsFreeCombo", { get: function() {
...
*/
Object.defineProperty(RPG_Skill.prototype, "jabsComboStarter", { get: function() {
...
*/
Object.defineProperty(RPG_Skill.prototype, "jabsAiSkillExclusion", { get: function() {
...
* @type {number|null}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsComboSkillId", { get: function() {
...
* @type {number}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsPierceCount", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsUnparryable", { get: function() {
...
* @type {boolean}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsOffhandEligible", { get: function() {
...
* @type {number}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsSelfAnimationId", { get: function() {
...
* @type {number}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsOnCastAnimationId", { get: function() {
...
* @type {number|null}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsDelayDuration", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsDelayTriggerByTouch", { get: function() {
...
* @type {number|null}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsDelayTriggerRadius", { get: function() {
...
* @type {number|null}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsVisZ", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsVisRotate", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_Skill.prototype, "jabsVisDebug", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_State.prototype, "jabsParalyzed", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_State.prototype, "jabsRooted", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_State.prototype, "jabsMuted", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_State.prototype, "jabsDisarmed", { get: function() {
...
* @type {number|null}
*/
Object.defineProperty(RPG_State.prototype, "jabsAggroInAmp", { get: function() {
...
* @type {number|null}
*/
Object.defineProperty(RPG_State.prototype, "jabsAggroOutAmp", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_State.prototype, "jabsAggroLock", { get: function() {
...
*/
Object.defineProperty(RPG_State.prototype, "jabsStateReapplyType", { get: function() {
	const type = RPGManager.getStringFromNoteByRegex(this, J.ABS.RegExp.ReapplyType);
	switch (type.toLowerCase()) {
		case JABS_State.reapplicationType.Refresh:
		case JABS_State.reapplicationType.Extend:
		case JABS_State.reapplicationType.Stack: return type;
		default: return null;
	}
} });
/**
...
*/
Object.defineProperty(RPG_State.prototype, "jabsStateRefreshDiminish", { get: function() {
	return RPGManager.getNumberFromNoteByRegex(this, J.ABS.RegExp.ReapplyRefreshDiminish, true) ?? J.ABS.Metadata.DefaultStateRefreshDiminish;
} });
/**
...
* Will either return the custom number of frames defined on the state, or the default from configuration.
*/
Object.defineProperty(RPG_State.prototype, "jabsStateRefreshReset", { get: function() {
	return RPGManager.getNumberFromNoteByRegex(this, J.ABS.RegExp.ReapplyRefreshReset, true) ?? J.ABS.Metadata.DefaultStateRefreshReset;
} });
/**
...
* Will either return the custom number of frames defined on the state, or the default from configuration.
*/
Object.defineProperty(RPG_State.prototype, "jabsStateExtendAmount", { get: function() {
	return RPGManager.getNumberFromNoteByRegex(this, J.ABS.RegExp.ReapplyExtendAmount, true) ?? J.ABS.Metadata.DefaultStateExtendAmount;
} });
/**
...
*/
Object.defineProperty(RPG_State.prototype, "jabsStateExtendMax", { get: function() {
	return RPGManager.getNumberFromNoteByRegex(this, J.ABS.RegExp.ReapplyExtendMax, true) ?? J.ABS.Metadata.DefaultStateExtendMax;
} });
/**
...
*/
Object.defineProperty(RPG_State.prototype, "jabsStateStackMax", { get: function() {
...
* Will either return the custom number of stacks defined on the state, or the default from configuration.
*/
Object.defineProperty(RPG_State.prototype, "jabsStateStacksApplied", { get: function() {
	return RPGManager.getNumberFromNoteByRegex(this, J.ABS.RegExp.StateApplicationAmount, true) ?? J.ABS.Metadata.DefaultStateApplicationCount;
} });
/**
...
* @type {boolean}
*/
Object.defineProperty(RPG_State.prototype, "jabsLoseAllStacksAtOnce", { get: function() {
...
* @type {number}
*/
Object.defineProperty(RPG_Traited.prototype, "jabsBonusHitsScopeGlobal", { get: function() {
	return RPGManager.getNumberFromNoteByRegex(this, J.ABS.RegExp.BonusHitsScopeGlobal);
} });
/**
...
* @type {number}
*/
Object.defineProperty(RPG_Traited.prototype, "jabsBonusHitsScopeBasic", { get: function() {
	return RPGManager.getNumberFromNoteByRegex(this, J.ABS.RegExp.BonusHitsScopeBasic);
} });
/**
...
* @type {number}
*/
Object.defineProperty(RPG_Traited.prototype, "jabsBonusHitsScopeSkill", { get: function() {
	return RPGManager.getNumberFromNoteByRegex(this, J.ABS.RegExp.BonusHitsScopeSkill);
} });

//#endregion
//#region src/plugins/abs/core/database/RPG_UsableItem.js
/**
* The JABS cooldown when using this skill or item.
* @type {number}
*/
Object.defineProperty(RPG_UsableItem.prototype, "jabsCooldown", { get: function() {
...
* @type {boolean|null}
*/
Object.defineProperty(RPG_UsableItem.prototype, "jabsUniqueCooldown", { get: function() {
...
* @type {boolean}
*/
Object.defineProperty(RPG_UsableItem.prototype, "jabsIgnoresGlobalCooldown", { get: function() {
...
* @type {boolean}
*/
Object.defineProperty(RPG_UsableItem.prototype, "jabsHiddenFromMenus", { get: function() {
	return RPGManager.checkForBooleanFromNoteByRegex(this, J.ABS.RegExp.HideFromJabsMenu);
} });

//#endregion
//#region src/plugins/abs/core/managers/DataManager.js
/**
...
DataManager.makeSaveContents = function() {
	const contents = J.ABS.Aliased.DataManager.get("makeSaveContents").call(this);
	/**
	* @type {Game_Event[]}
	*/
...
Game_Action.prototype.itemHit = function() {
	const successFactor = this.item().successRate * .01;
...
Game_Actor.prototype.canRefreshBasicAttackSkills = function() {
	...
	if (!this.getSkillSlotManager().isSetupComplete()) {
		return false;
	}
	return true;
};
...
Game_Actor.prototype.buildOffhandAssignableSkillIds = function() {
	...
	const explicitlyEligibleSkills = this.skills().filter(JABS_Battler.isSkillVisibleInOffhandMenu);
...
Game_Actor.prototype.removeInvalidSkills = function() {
	...
	const pinnedOffhandSkillId = this.getPinnedOffhandSkillId();
	if (pinnedOffhandSkillId && !this.isOffhandSkillAssignable(pinnedOffhandSkillId)) {
		this.getSkillSlotManager().clearOffhandPin();
	}
};
/**
...
Game_Actor.prototype.switchLocked = function() {
	...
	const switchLocked = objectsToCheck.some((object) => RPGManager.checkForBooleanFromNoteByRegex(object, J.ABS.RegExp.ConfigNoSwitch));
	return switchLocked;
};
/**
...
Game_Actor.prototype.getUpgradableSkillSlots = function() {
	...
	const upgradableSkillSlots = this.getValidEquippedSkillSlots().filter(filtering, this);
...
Game_Actor.prototype.canUpgradeSkill = function(skillSlot, skillId) {
	...
	const canAutoUpgrade = objectsToCheck.some((object) => RPGManager.checkForBooleanFromNoteByRegex(object, J.ABS.RegExp.ConfigAutoUpgradeSkills));
	if (!canAutoUpgrade) return false;
	const currentSkillData = this.skill(skillSlot.id);
	const isSkillAutoUpgradeBlocked = RPGManager.checkForBooleanFromNoteByRegex(currentSkillData, J.ABS.RegExp.NoSkillUpgrading);
	if (isSkillAutoUpgradeBlocked) return false;
	const upgradeOverThisSkillId = RPGManager.getNumberFromNoteByRegex(this.skill(skillId), J.ABS.RegExp.UpgradeOverSkill);
...
Game_Actor.prototype.canAutoAssignSkillOnLevelup = function(skillId) {
	const objectsToCheck = this.getActorNotes();
	const canAutoAssign = objectsToCheck.some((object) => RPGManager.checkForBooleanFromNoteByRegex(object, J.ABS.RegExp.ConfigAutoAssignSkills));
...
Game_Battler.prototype.basicAttackSkillId = function() {
	...
	const attackSkillTrait = databaseData.traits.find((trait) => trait.code === J.BASE.Traits.ATTACK_SKILLID);
	if (attackSkillTrait) {
...
Game_Battler.prototype.isSlotLocked = function(slot) {
	return this.getSkillSlotManager().getSkillSlotByKey(slot).isLocked();
};
/**
...
Game_Battler.prototype.unlockSlot = function(slot) {
	this.getSkillSlotManager().getSkillSlotByKey(slot).unlock();
};
/**
...
Game_Battler.prototype.getStateDurationBoost = function(baseDuration) {
	...
	const formulaiBoost = RPGManager.getResultsFromAllNotesByRegex(objectsToCheck, J.ABS.RegExp.StateDurationFormulaPlus, baseDuration, this);
...
Game_Battler.prototype.resolveEquippedSkillId = function(baseSkillId) {
	...
	for (const source of sources) {
		...
		const match = source.jabsSkillTransforms.find((transform) => {
...
//#endregion
//#region src/plugins/abs/core/objects/Game_CharacterBase.js
/**
...
Game_CharacterBase.prototype.getDashSpeedBoost = function() {
	return this.isDashing() ? this.dashSpeed() : 0;
};
/**
...
Game_CharacterBase.prototype.getDodgeSpeedModifier = function() {
	return this.isDodging() ? this.dodgeModifier() : 0;
};
/**
...
//#endregion
//#region src/plugins/abs/core/objects/Game_Interpreter.js
/**
...
//#endregion
//#region src/plugins/abs/core/objects/Game_Map.js
/**
...
Game_Map.prototype.getJabsLootDrops = function() {
	return this.events().filter((event) => event.isJabsLoot());
};
/**
...
//#endregion
//#region src/plugins/abs/core/objects/Game_Party.js
/**
...
//#endregion
//#region src/plugins/abs/core/objects/Game_Player.js
/**
...
var Window_AbsMenu = class extends Window_Command {
	...
	*/
	buildCommands() {
		...
		const offhandSkillCommand = new WindowCommandBuilder(J.ABS.Metadata.EquipOffhandText).setSymbol("offhand-assign").setEnabled(true).setIconIndex(81).setColorIndex(4).setHelpText(this.offhandSkillHelpText()).build();
	...
	*/
	combatSkillsHelpText() {
		const description = ["The `Combat Skills` are more powerful variants of your basic attacks that may require resources to execute.", "Typical things like sword techs and magic spells will show up here."];
		return description.join("\n");
	}
	/**
	...
	*/
	dodgeSkillHelpText() {
		const description = ["The `Dodge Skills` are ones that grant some form of mobility.", "It is encouraged to use these liberally to maneuver around the field, in and out of combat."];
		return description.join("\n");
	}
	/**
	...
	*/
	toolHelpText() {
		const description = ["Your tool list, where you can find any and all equippable items.", "Not all items will show up in the list- only ones usable in combat somehow will be available."];
		return description.join("\n");
	}
	/**
	* Closes the Abs menu.
...
//#endregion
//#region src/plugins/abs/core/windows/Window_AbsMenuSelect.js
/**
...
var Window_AbsMenuSelect = class Window_AbsMenuSelect extends Window_Command {
	...
	*/
	makeCombatSkillList() {
		...
		const forEacher = (skill) => {
			...
			const skillCommand = new WindowCommandBuilder(name).setSymbol("skill").setExtensionData(id).setIconIndex(iconIndex).setHelpText(description).setTextLines(description.split(/[\r\n]+/)).build();
	...
	*/
	makeToolList() {
		const commands = Array.empty;
		const clearSlotCommand = new WindowCommandBuilder(J.ABS.Metadata.ClearSlotText).setSymbol("tool").setTextLines(["Remove the existing tool from the slot."]).setColorIndex(16).build();
		commands.push(clearSlotCommand);
		const forEacher = (tool) => {
			const { name, id, iconIndex, description } = tool;
			const amount = tool.consumable ? $gameParty.numItems(tool).padZero(3) : "♾";
	...
	makeEquippedCombatSkillList() {
		...
		*/
		const forEacher = (skillSlot) => {
			...
			const command = new WindowCommandBuilder(name).setSymbol("slot").setExtensionData(skillSlot.key).setIconIndex(iconIndex).build();
	...
	*/
	makeEquippedToolList() {
		...
		if (toolSkillSlot.isUsable()) {
			...
			amount = equippedTool.consumable ? $gameParty.numItems(equippedTool).padZero(3) : "♾";
			name = equippedTool.name;
			iconIndex = equippedTool.iconIndex;
			description = equippedTool.description;
		}
		const command = new WindowCommandBuilder(name).setSymbol("slot").setExtensionData(toolSkillSlot.key).setIconIndex(iconIndex).setRightText(`x${amount}`).build();
		this.addBuiltCommand(command);
	}
	/**
	* Fills the list with the currently assigned items.
	*/
	makeEquippedDodgeSkillList() {
		...
		const command = new WindowCommandBuilder(name).setSymbol("slot").setExtensionData(dodgeSkillSlot.key).setIconIndex(iconIndex).build();
		this.addBuiltCommand(command);
	}
	/**
	...
	*/
	makeOffhandSkillList() {
		const commands = Array.empty;
		const clearSlotCommand = new WindowCommandBuilder("Use Equipment Default.").setSymbol("offhand").setColorIndex(16).setTextLines(["Remove the offhand pin and let the equipped offhand grant the skill again."]).build();
		commands.push(clearSlotCommand);
		const offhandSkills = $gameParty.leader().buildOffhandAssignableSkillPool();
		const forEacher = (offhandSkill) => {
			const { name, id, iconIndex, description } = offhandSkill;
			const offhandCommand = new WindowCommandBuilder(name).setSymbol("offhand").setExtensionData(id).setIconIndex(iconIndex).setTextLines(description.split(/[\r\n]+/)).build();
...
Scene_Map.prototype.commandOffhand = function() {
	...
	this.getJabsOffhandSkillListWindow().refresh();
	this.getJabsEquippedOffhandSkillWindow().refresh();
	this.showJabsEquippedOffhandSkillWindow();
	this.getJabsEquippedOffhandSkillWindow().deselect();
	this.getJabsEquippedOffhandSkillWindow().deactivate();
...
//#endregion
//#region src/plugins/abs/core/sprites/Sprite_Animation.js
/**
...
Sprite_Animation.prototype.setup = function(targets, animation, mirror, delay, previous) {
	if (animation === undefined || animation === null) {
		J.ABS.Aliased.Sprite_Animation.get("setup").call(this, targets, animation, mirror, delay, previous);
		return;
	}
	const hadSoundArray = Array.isArray(animation.soundTimings);
	const hadFlashArray = Array.isArray(animation.flashTimings);
	const soundTimings = hadSoundArray ? animation.soundTimings.filter((t) => t !== undefined && t !== null) : [];
	const flashTimings = hadFlashArray ? animation.flashTimings.filter((t) => t !== undefined && t !== null) : [];
	const arraysAlreadyDense = hadSoundArray && hadFlashArray && soundTimings === animation.soundTimings && flashTimings === animation.flashTimings;
	if (arraysAlreadyDense === true) {
		J.ABS.Aliased.Sprite_Animation.get("setup").call(this, targets, animation, mirror, delay, previous);
		return;
	}
	const safeAnimation = Object.assign({}, animation, {
		soundTimings,
		flashTimings
	});
	J.ABS.Aliased.Sprite_Animation.get("setup").call(this, targets, safeAnimation, mirror, delay, previous);
};
/**
...
//#endregion
//#region src/plugins/abs/core/sprites/Sprite_MapCastGauge.js
/**
...
//#endregion
//#region src/plugins/abs/core/sprites/Sprite_MapHpGauge.js
/**
...
Sprite_Character.prototype.isEmptyCharacter = function() {
	return this.isLoot() ? false : J.ABS.Aliased.Sprite_Character.get("isEmptyCharacter").call(this);
};
/**
...
Sprite_Character.prototype.updatePosition = function() {
	J.ABS.Aliased.Sprite_Character.get("updatePosition").call(this);
	if (!this.isJabsAction() || this.character().isErased()) {
		return;
	}
	if (this.character().getJabsActionNeedsRemoving()) {
		return;
	}
...
Sprite_Character.prototype.applyActionVisuals = function() {
	const character = this.character();
	const jabsAction = character.getJabsAction();
	if (!jabsAction) return;
	const skill = jabsAction.getBaseSkill();
	if (!skill) return;
...
Sprite_Character.prototype.applyActionAnchor = function(skill) {
	...
	const visAnchor = skill.getJabsVisAnchorMergedForActionMap(jabsAction);
	if (!visAnchor) return;
...
Sprite_Character.prototype.directionToRadians = function(dir) {
	...
	switch (dir) {
		case 2: return RAD_0;
		case 3: return RAD_45;
		case 6: return RAD_90;
		case 9: return RAD_90 + RAD_45;
		case 8: return RAD_180;
		case 7: return -RAD_90 - RAD_45;
		case 4: return RAD_N90;
		case 1: return RAD_N45;
	}
	return 0;
};
/**
...
Sprite_Character.prototype.applyActionScale = function(skill) {
	const jabsAction = this.character().getJabsAction();
	const visScale = skill.getJabsVisScaleMergedForActionMap(jabsAction);
	if (!visScale) return;
...
Sprite_Character.prototype.applyActionDebug = function(skill) {
	...
	if (skill.getJabsVisDebugMergedForActionMap(jabsAction)) {
		this._j._abs._visDebugGizmo ||= this.createJabsVisDebugGizmo();
...
Sprite_Character.prototype.computeMapTierStripeY = function(textSprite, outerH) {
	const textH = textSprite.bitmap ? textSprite.bitmap.height : textSprite.fontSize() * 3;
...
//#endregion
//#region src/plugins/abs/core/sprites/Sprite_Gauge.js
/**
...
Spriteset_Map.prototype.collectActiveCastPreviewItems = function() {
	...
	const items = [];
	$gameMap.events().filter((ev) => ev.isJabsBattler()).forEach((ev) => {
	...
	Object.keys(dict).forEach((key) => {
		...
		if (item.action.isDirectAction && item.action.isDirectAction()) {
			...
			if (tx !== null && ty !== null) {
				const tw = $gameMap.tileWidth();
				const th = $gameMap.tileHeight();
				screenX = Math.round(($gameMap.adjustX(tx) + .5) * tw);
				screenY = Math.round(($gameMap.adjustY(ty) + .5) * th);
			}
		}
		sprite.x = screenX;
		sprite.y = screenY;
...
Spriteset_Map.prototype.createCastPreviewSprite = function(item) {
	...
	/** @type {PIXI.Graphics} */
	const g = new PIXI.Graphics();
	sprite._jabsCastPreviewG = g;
	sprite._cpKey = item.key;
	sprite.addChild(g);
...
Spriteset_Map.prototype.handleHitboxOverlay = function() {
	...
	const pulseColliderHighlight = J.ABS.Metadata.HitboxPulse.enabled === true && J.ABS.Metadata.HitboxPulse.highlightColliderBattlers === true;
...
Spriteset_Map.prototype.clearAllHitboxOverlays = function() {
	const layer = this.getJabsHitboxLayer();
	const actionDict = this.getActionHitboxSprites();
	const battlerDict = this.getBattlerHitboxSprites();
...
Spriteset_Map.prototype.applyHitboxStyle = function(g, style) {
	...
	g.lineStyle(style.lineWidth, style.lineColor, style.lineAlpha, lineAlignment);
...
Spriteset_Map.prototype.isBattlerCollidingWithAnyAction = function(item) {
	...
	const actions = $gameMap.actionEvents();
	for (let i = 0; i < actions.length; i++) {
		const actionEvent = actions[i];
		const jabsAction = actionEvent.getJabsAction();
		if (!jabsAction) continue;
		const shape = jabsAction.getShape();
		const range = jabsAction.getRange();
		...
		if (jabsAction.isDirectAction()) {
			...
			const gameAction = jabsAction.getAction();
			if (gameAction.isForUser()) {
				const casterChar = jabsAction.getCaster().getCharacter();
...
Spriteset_Map.prototype.getActionHitboxStyleFor = function(shape) {
	...
	const styles = J.ABS.Metadata.HitboxStyles || {};
	const base = Object.assign({}, defaults, styles.base || {});
	const key = (shape || String.empty).toLowerCase();
	const shapeOverrides = (styles.byShape || {})[key] || null;
	const finalStyle = Object.assign({}, base, shapeOverrides || {});
	return finalStyle;
};
/**
...
Spriteset_Map.prototype.buildMissingActionHitboxSprites = function() {
	...
	actions.forEach((actionEvent) => {
		const key = actionEvent.getJabsActionUuid();
		if (!key) return;
		if (dict[key]) return;
		const sprite = this.createActionHitboxSprite(actionEvent);
		dict[key] = sprite;
		layer.addChild(sprite);
	});
};
...
Spriteset_Map.prototype.getOrCreateActionHitboxSpriteFor = function(actionEvent) {
	...
	const sprite = this.createActionHitboxSprite(actionEvent);
	dict[key] = sprite;
	this.getJabsHitboxLayer().addChild(sprite);
	return sprite;
};
/**
...
Spriteset_Map.prototype.createActionHitboxSprite = function(actionEvent) {
	...
	/** @type {PIXI.Graphics} */
	const g = new PIXI.Graphics();
	sprite._jabsHitboxG = g;
	sprite._actionUuid = actionEvent.getJabsActionUuid();
	sprite.addChild(g);
	sprite.anchor.set(.5, .5);
	return sprite;
};
...
* @returns {void}
*/
Spriteset_Map.prototype.drawOrientedHitboxQuadG = function(g, facing, lengthPx, halfBreadthPx) {
...
Spriteset_Map.prototype.drawActionHitboxInto = function(sprite, shape, range, facing, tw, th, actionEvent) {
	...
	g.clear();
	const style = this.getActionHitboxStyleFor(shape);
	this.applyHitboxStyle(g, style);
...
Spriteset_Map.prototype.buildMissingBattlerHitboxSprites = function(itemMode = "all") {
	...
	const items = this.collectBattlerOverlayItems(itemMode);
	items.forEach((item) => {
		if (dict[item.key]) return;
...
Spriteset_Map.prototype.refreshExistingBattlerHitboxSprites = function(itemMode = "all") {
	...
	const items = this.collectBattlerOverlayItems(itemMode);
	items.forEach((item) => {
		const sprite = this.getOrCreateBattlerHitboxSpriteFor(item);
		const cx = item.source.screenX();
		const cy = item.source.screenY();
		sprite.x = cx;
		sprite.y = cy;
		const aabb = JABS_Engine.getBattlerAabbModel(item.source);
		const colliding = this.isBattlerCollidingWithAnyAction(item);
		this.drawBattlerHitboxInto(sprite, item.type, tw, th, colliding, aabb);
	});
};
/**
...
Spriteset_Map.prototype.createBattlerHitboxSprite = function(item) {
	...
	const g = new PIXI.Graphics();
	sprite._jabsHitboxG = g;
	sprite._battlerKey = item.key;
	sprite._battlerType = item.type;
	sprite.addChild(g);
	sprite.anchor.set(.5, .5);
	return sprite;
};
...
Spriteset_Map.prototype.drawBattlerHitboxInto = function(sprite, type, tw, th, colliding, aabb) {
	...
	const g = sprite._jabsHitboxG;
	g.clear();
	const style = this.getBattlerHitboxStyle(type, colliding ? "colliding" : null);
	this.applyHitboxStyle(g, style);
...
Spriteset_Map.prototype.getBattlerHitboxStyle = function(kind, state) {
	...
	const styles = J.ABS.Metadata.HitboxStyles || {};
	const base = Object.assign({}, defaults, styles.base || {});
	const kindKey = (kind || "battler").toLowerCase();
	const byKind = (styles.byKind || {})[kindKey] || null;
	const stateKey = (state || String.empty).toLowerCase();
	const byState = stateKey ? (styles.byState || {})[stateKey] || null : null;
	const finalStyle = Object.assign({}, base, byKind || {}, byState || {});
	return finalStyle;
};
/**
