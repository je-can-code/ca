...
//#endregion
//#region src/plugins/abs/ext/food/windows/Window_AbsMenu.js
/**
...
Window_AbsMenu.prototype.buildCommands = function() {
	...
	const foodCommand = new WindowCommandBuilder(J.ABS.EXT.FOOD.Metadata.EquipFoodText).setSymbol("food-assign").setEnabled(true).setIconIndex(210).setColorIndex(29).setHelpText(this.foodAssignHelpText()).build();
...
Window_AbsMenu.prototype.foodAssignHelpText = function() {
	const lines = ["Select a food item to assign to the R2 food slot.", "Consuming food applies buffet heals and starts a typed chain arc."];
	return lines.join("\n");
};

//#endregion
//#region src/plugins/abs/ext/food/windows/Window_AbsMenuSelect.js
/**
...
Window_AbsMenuSelect.prototype.makeFoodList = function() {
	...
	const clearSlotCommand = new WindowCommandBuilder(J.ABS.Metadata.ClearSlotText).setSymbol("food").setColorIndex(16).setTextLines(["Remove the currently equipped food item from the slot."]).build();
	commands.push(clearSlotCommand);
	const forEacher = (foodItem) => {
		const { name, id, iconIndex, description } = foodItem;
		const amount = foodItem.consumable ? $gameParty.numItems(foodItem).padZero(3) : "♾";
		const foodCommand = new WindowCommandBuilder(name).setSymbol("food").setExtensionData(id).setIconIndex(iconIndex).setHelpText(description).setRightText(`x${amount}`).setTextLines(description.split(/[\r\n]+/)).build();
		commands.push(foodCommand);
	};
	const foodItems = $gameParty.allItems().filter((item) => this.isItemVisibleInFoodMenu(item));
...
Window_AbsMenuSelect.prototype.makeEquippedFoodList = function() {
	...
	const equippedFood = foodSkillSlot.isUsable() ? $dataItems.at(foodSkillSlot.id) : RPG_BaseItem.Empty;
	const name = equippedFood.id > 0 ? equippedFood.name : `${foodSkillSlot.key}: ${J.ABS.Metadata.UnassignedText}`;
	const { iconIndex, description } = equippedFood;
	let amount = String.empty;
	if (equippedFood.id > 0) {
		amount = equippedFood.consumable ? $gameParty.numItems(equippedFood).padZero(3) : "♾";
	}
	const command = new WindowCommandBuilder(name).setSymbol("slot").setExtensionData(foodSkillSlot.key).setIconIndex(iconIndex).setRightText(`x${amount}`).setHelpText(description).build();
...
//#endregion
//#region src/plugins/abs/ext/food/scenes/Scene_Map.js
/**
...
*/
J.ABS.EXT.FOOD.Aliased.Scene_Map.set("redirectToParentAssignMenu", Scene_Map.prototype.redirectToParentAssignMenu);
