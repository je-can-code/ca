 ...
 * @target MZ
 * @plugindesc
 * [v2.0 TARGET] Adds a targeting window to the map.
 * @author JE
...
Game_Enemy.prototype.setup = function(enemyId, x, y)
{
  ...
  {
    if (this.notetags()[i] == "Hide HP")
    {
      this._chronoEhp_meter = false
    }
    ;
  }
  ;
...
Game_Action.prototype.chronoEnemyHPNeedData = function(target)
{
  ...
  ;
  if (target.isEnemy() && !target._chronoEhp_meter)
  {
    return false
  }
  ;
...
{
  if (Moghunter.chrono_enemyHP_duration < 20)
  {
    return 20
  }
...
CNHPGauge.prototype.refreshHP = function(init)
{
  ...
  if (this._enemyData[0] !== this.eName())
  {
    this.refreshName()
  }
  this.refreshBlueHP();
...
CNHPGauge.prototype.updateRedMeter = function()
{
  ...
  {
    this._hpOld[0] -= dnspeed;
    if (this._hpOld[0] < this.hp())
    {
      this._hpOld[0] = this.hp()
    }
  }
  else if (this._hpOld[0] < this.hp())
  {
    this._hpOld[0] += dnspeed;
    if (this._hpOld[0] > this.hp())
    {
